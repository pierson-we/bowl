...
def Unet(img_size, img_channels, num_classes):
...
def DeepLab(img_size, img_channels, num_classes):
    ...
    def DeeplabV2(img_size, img_channels, upsampling=8, apply_softmax=True,
                  input_tensor=None,
                  classes=num_classes):
        ...
        # out = Conv2D(1, (1,1), activation='softmax')(logits)
        ...
        return model
    return DeeplabV2(img_size, img_channels)


# from https://github.com/Borda/segment-nuclei/blob/master/code/helper/model_builder.py

def random_masters(img_size, img_channels, num_classes):
    import keras.layers
    import keras.models
    ...
    def get_model_1_class(dim1, dim2, num_classes):
    ...
    return get_model_1_class(img_size, img_size, num_classes)

# from https://github.com/Vladkryvoruchko/PSPNet-Keras-tensorflow/blob/master/layers_builder.py

def PSPnet(img_size, img_channels, num_classes):
    ...
    def build_pspnet(nb_classes, resnet_layers, input_shape, img_channels, activation='softmax'):
        """Build PSPNet."""
        ...
        return model
    return build_pspnet(nb_classes=num_classes, resnet_layers=50, input_shape=(img_size, img_size), img_channels=img_channels)
