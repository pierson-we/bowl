...
def make_2_class(mask):
    mask = np.reshape(mask, (mask.shape[0], mask.shape[1]))
    # borders_2 = make_borders(mask)
    # borders_2[0, :] = 0
    # borders_2[-1, :] = 0
    # borders_2[:, 0] = 0
    # borders_2[:, -1] = 0
    bg_3 = np.where(mask == 0, 1, 0)
    # fg_1 = np.where((np.where(mask >= 0.5, 1, 0) - borders_2) == 1, 1, 0)
    fg_1 = mask
    # print(bg_3.shape)
    # print(borders_2.shape)
    # print(fg_1.shape)
    
    # fig, m_axs = plt.subplots(nrows=2, ncols=3, figsize = (12, 6))
    # for c_im1, c_im2, c_im4 in m_axs:
    #     c_im1.imshow(mask)
    #     c_im1.axis('off')
    ...
    #     # c_im3.imshow(borders_2)
    #     # c_im3.axis('off')
    #     # c_im3.set_title('borders')
    ...
    #     plt.show()
    # sys.exit()
    return np.stack((fg_1, bg_3), axis=2)

def format_mask(mask, img, _id, class_df, num_classes):
    ...
    mask_multiclass[:,:,img_class] = mask
    mask_multiclass[:,:,int(img_class+(num_classes/2))] = bg
    return mask_multiclass

def make_df(train_path, test_path, img_size, classes_csv):
    ...
    for i, id_ in enumerate(train_ids):
        ...
        for mask_file in next(os.walk(path + '/masks/'))[2]:
            ...
            mask = np.maximum(mask, format_mask(mask_, img, id_, class_df, num_classes))
            # mask = np.maximum(mask, mask_)
        Y_train[i] = mask # make_2_class(mask) # make_3_class(mask)
...
def prob_to_rles(x, cut_off = 0.2):
...
def resize_masks(row):
    row['masks_resized'] = np.where(transform.resize(image=row['masks'], output_shape=(row['shape'][0],row['shape'][1]), preserve_range=True, mode='constant') >= 0.2, 1, 0)
    return row
