...
if __name__ == "__main__":
    ...
    model = models.PSPnet(img_size, img_channels, num_classes)
    ...
    # LR = callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, verbose=0, mode='auto', epsilon=0.05, cooldown=0, min_lr=0)
    ...
    model.compile(optimizer=sgd, # RMSprop(lr = 1e-4),
                  loss='categorical_crossentropy', # losses.bce_dice_loss,
                  metrics=[losses.mean_iou, losses.dice_coef, losses.precision, 'acc'])

    # model.fit_generator(train_generator, steps_per_epoch=len(xtr)/(batch_size), epochs=20,
    #                     validation_data=val_generator, validation_steps=len(xval)/batch_size, callbacks=[checkpoint]) # , LR])
    
    test_img_df = test_img_df.iloc[5:10]
    test_img_df.head()
    test_img_df['masks_multiclass'] = test_img_df['images'].map(lambda x: model.predict(np.expand_dims(x, 0))[0, :, :, :])
    # test_img_df['masks'] = test_img_df['masks_multiclass'].map(utils.masks_1_class) # utils.masks_1_class)
    test_img_df = utils.decluster_masks(test_img_df)
    ...
    n_img = 4
    row = 0
    for i in range(0, 20): # (test_img_df.shape[0]/(n_img*3) + 1)):   
        fig, m_axs = plt.subplots(3, n_img, figsize = (12, 6))
        for c_im1, c_im2, c_im4, c_im5 in m_axs:
            # print(test_img_df.iloc[row]['masks_multiclass'].shape)
            c_im1.imshow(np.reshape(test_img_df.iloc[row]['masks'], (img_size, img_size)))
            c_im1.axis('off')
            # c_im1.set_title('microscope')

            c_im2.imshow(np.reshape(test_img_df.iloc[row]['masks_crf'], (img_size, img_size)))
            c_im2.axis('off')
            # c_im2.set_title('image raw')
            ...
            c_im4.imshow(utils.clean_img(test_img_df.iloc[row]['masks_resized']))
            c_im4.axis('off')
